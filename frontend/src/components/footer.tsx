        ...
        <div className="grid gap-16 lg:grid-cols-12">
          ...
          <div className="lg:col-span-4 space-y-10">
            ...
            <p className="text-stone-500 max-w-sm text-lg leading-relaxed font-bold">
              Providing transparent real estate data for strategic and private investment decisions across the European horizon.
            </p>
            <div className="flex items-center gap-5">
              <Link href="#" className="size-11 rounded-xl bg-white border border-stone-200 flex items-center justify-center text-stone-400 hover:text-[#34495E] hover:border-[#34495E]/20 transition-all shadow-sm">
          ...
          <div className="lg:col-span-5 grid grid-cols-2 md:grid-cols-3 gap-12">
            {links.map((link) => (
              <div key={link.group} className="space-y-6">
                <h3 className="text-[10px] font-black uppercase tracking-widest text-[#2C3E50] border-b border-stone-200 pb-4">
                  {link.group}
                </h3>
                <ul className="space-y-4">
                  {link.items.map((item) => (
                    <li key={item.title}>
                      <Link
                        href={item.href}
                        className="text-sm font-bold text-stone-500 hover:text-[#34495E] transition-colors inline-flex items-center group/link"
                      >
                        {item.title}
