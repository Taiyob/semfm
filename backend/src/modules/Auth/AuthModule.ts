...
export class AuthModule extends BaseModule {
    ...
    public readonly basePath = '/api/auth';
    ...
    /**
     * Setup module use cases / services
     */
    protected async setupUseCases(): Promise<void> {
        ...
        // Initialize services
        this.authService = new AuthService(this.context.getService('prisma'));
    ...
    protected async setupRoutes(): Promise<void> {
        ...
        // Mount routes (BaseModule automatically prefixes with this.basePath)
    ...
    protected async onBeforeInit(): Promise<void> {
        ...
        try {
            ...
            const sampleUser = await this.context.getService('prisma').user.findFirst({
    ...
    public async healthCheck(): Promise<{ status: 'healthy' | 'unhealthy'; details?: any }> {
        try {
            ...
            const userCount = await this.context.getService('prisma').user.count();

            // Check JWT configuration
            const jwtConfigured = !!config.security.jwt.secret;

            // Get user statistics
            const activeUsers = await this.context.getService('prisma').user.count({
                where: { status: 'active' },
            });

            const adminUsers = await this.context.getService('prisma').user.count({
    ...
    private async createDefaultAdmin(): Promise<void> {
        try {
            const adminCount = await this.context.getService('prisma').user.count({
                where: { role: { name: 'admin' } },
            });
    ...
    public async getModuleStats() {
        try {
            const [totalUsers, activeUsers, adminUsers] = await Promise.all([
                this.context.getService('prisma').user.count(),
                this.context.getService('prisma').user.count({ where: { status: 'active' } }),
                this.context.getService('prisma').user.count({ where: { role: { name: 'admin' } } }),
            ]);
