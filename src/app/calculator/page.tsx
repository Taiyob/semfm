  ...
  useEffect(() => {
    ...
    if (price || rent || location) {
      setFormData(prev => ({
        ...
        neighborhood: location || prev.neighborhood,
      ...
      <div className="grid lg:grid-cols-12 gap-16">
              ...
              {steps.map((s, i) => (
                <div key={s.id} className="flex items-center gap-2 shrink-0">
                  <button 
                    ...
                    className={`p-4 rounded-2xl flex items-center gap-3 transition-all ${step === s.id ? 'bg-[#34495E] text-white shadow-xl shadow-[#34495E]/20' : 'bg-stone-100 text-stone-400'}`}
                  >
              ...
              <AnimatePresence mode="wait">
                  ...
                  <motion.div key="step-1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-10">
                    ...
                    <div className="space-y-6">
                        <div className="grid md:grid-cols-3 gap-4">
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-stone-400 ml-2">Neighborhood</label>
                                <input type="text" value={formData.neighborhood} onChange={(e) => setFormData({...formData, neighborhood: e.target.value})} placeholder="e.g. Arroios" className="w-full bg-stone-50 rounded-2xl p-4 font-black outline-none border-2 border-transparent focus:border-[#34495E]" />
                            </div>
                        </div>
                    ...
                    {mode === 'advanced' && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="space-y-8 pt-4 border-t border-stone-50">
                            <div className="grid md:grid-cols-3 gap-6">
                                ...
                                <div className="space-y-3">
                                    <label className="text-xs font-bold text-stone-400 ml-2">Finishing quality</label>
                                    <select value={formData.finishing} onChange={(e) => setFormData({...formData, finishing: e.target.value})} className="w-full bg-stone-50 rounded-2xl p-4 font-black outline-none border-2 border-transparent focus:border-[#34495E]">
                                ...
                                <div className="space-y-3">
                                    <label className="text-xs font-bold text-stone-400 ml-2">Energy label</label>
                                    <select value={formData.energyLabel} onChange={(e) => setFormData({...formData, energyLabel: e.target.value})} className="w-full bg-stone-50 rounded-2xl p-4 font-black outline-none border-2 border-transparent focus:border-[#34495E]">
                            ...
                            <div className="grid md:grid-cols-3 gap-6">
                                <div className="flex items-center gap-4 p-4 bg-stone-50 rounded-2xl">
                                    <input type="checkbox" id="balcony" checked={formData.hasBalcony} onChange={(e) => setFormData({...formData, hasBalcony: e.target.checked})} className="size-5 accent-[#34495E]" />
                                    <label htmlFor="balcony" className="text-xs font-bold text-stone-900">Balcony</label>
                                </div>
                                <div className="flex items-center gap-4 p-4 bg-stone-50 rounded-2xl">
                                    <input type="checkbox" id="parking" checked={formData.hasParking} onChange={(e) => setFormData({...formData, hasParking: e.target.checked})} className="size-5 accent-[#34495E]" />
                                    <label htmlFor="parking" className="text-xs font-bold text-stone-900">Parking</label>
                                </div>
                                <div className="space-y-3">
                                    <label className="text-xs font-bold text-stone-400 ml-2">Elevator</label>
                                    <select value={formData.hasElevator} onChange={(e) => setFormData({...formData, hasElevator: e.target.value})} className="w-full bg-stone-50 rounded-2xl p-4 font-black outline-none border-2 border-transparent focus:border-[#34495E]">
                ...
                {step === 'gross_yield' && (
                  <motion.div key="step-2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-10">
                    <div>
                        <h4 className="text-xl font-black text-stone-900 mb-2 tracking-tight">Stage 2: Acquisition & gross yield</h4>
                        <p className="text-stone-500 text-sm font-bold italic">Calculating market entry cost vs revenue potential.</p>
                    </div>
                    
                    <div className="grid md:grid-cols-2 gap-8">
                      <div className="space-y-3">
                        <label className="text-[10px] font-black tracking-tight text-stone-400 ml-2">Total Buying Price (€)</label>
                        <input type="number" value={formData.purchasePrice} onChange={(e) => setFormData({ ...formData, purchasePrice: Number(e.target.value) })}
                          className="w-full bg-stone-50 border-2 border-transparent rounded-2xl px-6 py-5 outline-none focus:ring-2 focus:ring-[#34495E] focus:bg-white transition-all text-2xl font-black"
                        />
                      </div>
                      <div className="space-y-3">
                        <label className="text-[10px] font-black tracking-tight text-stone-400 ml-2">Expected Market Rent (€/mo)</label>
                    ...
                    <div className="p-10 bg-stone-950 rounded-[40px] text-white flex items-center justify-between border-b-4 border-[#D4A373]">
                         <div className="space-y-2">
                            <span className="text-[10px] font-black tracking-tight text-stone-500">Gross yield (pre-tax)</span>
                            <div className="text-5xl font-black text-[#D4A373]">{results.grossYield}%</div>
                         </div>
                         <ArrowUpRight className="size-12 text-[#34495E]" />
                    </div>
        ...
        <div className="lg:col-span-5">
            <div className="sticky top-32 space-y-8">
                <div className="bg-stone-950 p-10 lg:p-14 rounded-[56px] text-white shadow-2xl shadow-stone-900/40 relative overflow-hidden h-full">
                    <div className="relative z-10 space-y-10">
                        <div className="flex items-center gap-3">
                            <div className="size-10 bg-[#34495E] rounded-xl flex items-center justify-center">
                                <BarChart2 className="size-6 text-white" />
                            </div>
                            <span className="font-black tracking-tight text-[10px] text-stone-500 underline underline-offset-4 decoration-[#34495E]">Live horizon engine v4</span>
                        </div>

                        <div className="space-y-3 pb-10 border-b border-white/5">
                            <span className="text-[10px] font-black text-stone-500 tracking-tight">Phase 1: Estimated market rent</span>
                            <div className="text-5xl font-black tracking-tighter text-[#D4A373]">
                                €{formData.estimatedRent}<span className="text-2xl text-stone-700">/mo</span>
                            </div>
                        </div>

                        <div className="space-y-3 pb-10 border-b border-white/5">
                            <span className="text-[10px] font-black text-stone-500 tracking-tight">Phase 2: Est. gross yield</span>
                            <div className="text-7xl font-black tracking-tighter text-[#34495E]">
                        ...
                        <div className="grid grid-cols-2 gap-10">
                            <div className="space-y-2">
                                <span className="text-[10px] font-black text-stone-600 tracking-tight">Cash ROI</span>
                                <GatedData><div className="text-3xl font-black text-[#34495E]">{results.cashOnCash}%</div></GatedData>
                            </div>
                            <div className="space-y-2 text-right">
                                <span className="text-[10px] font-black text-stone-600 tracking-tight">20Y Forecast</span>
                                <GatedData><div className="text-3xl font-black text-[#D4A373]">+{results.combinedReturn}%</div></GatedData>
                            </div>
                        </div>


                        <Link href="/pricing" className="block w-full text-center">
                            <button className="btn-primary w-full !bg-[#FAF9F6] !text-stone-900 tracking-tight text-[11px] py-6 shadow-2xl group relative overflow-hidden">
                ...
                <div className="p-10 bg-[#34495E]/5 rounded-[40px] border border-[#34495E]/10 flex gap-6">
                    <PieChart className="size-10 text-[#34495E] shrink-0" />
                    <div>
                        <h6 className="font-black text-stone-900 mb-2 tracking-tight">Stage logic tracking</h6>
                        <p className="text-[10px] font-bold text-stone-500 leading-relaxed tracking-tight">
                            Market intensity in <span className="text-[#34495E]">{formData.region}</span> remains high for <span className="text-[#34495E]">{formData.bedrooms} Bedroom</span> units.
                        </p>
                    </div>
                </div>
            </div>
