    ...
    <div className="min-h-screen pt-32 pb-24 font-outfit hero-gradient">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center max-w-4xl mx-auto mb-20 space-y-8">
            ...
            <h1 className="text-5xl md:text-8xl font-black text-stone-900 leading-[0.9] tracking-tight uppercase">High Fidelity <br /><span className="gradient-text">Analysis Access.</span></h1>
            ...
            <div className="flex items-center justify-center gap-6 pt-10">
                <span className={`text-[10px] font-black uppercase tracking-widest ${billing === 'monthly' ? 'text-[#B55D3E]' : 'text-stone-400'}`}>Monthly Matrix</span>
                <button 
                  onClick={() => setBilling(billing === 'monthly' ? 'yearly' : 'monthly')}
                  className="w-16 h-9 bg-stone-900 rounded-full p-1.5 relative transition-all"
                >
                    <div className={`size-6 bg-[#B55D3E] rounded-full transition-all duration-300 shadow-lg ${billing === 'yearly' ? 'translate-x-7' : 'translate-x-0'}`} />
                </button>
                <div className="flex items-center gap-3">
                    <span className={`text-[10px] font-black uppercase tracking-widest ${billing === 'yearly' ? 'text-[#D4A373]' : 'text-stone-400'}`}>Annual Protocol</span>
                    <span className="text-[10px] font-black bg-[#B55D3E] text-white px-3 py-1 rounded-lg uppercase tracking-widest animate-pulse">Save 20%</span>
                </div>
            </div>
        </div>
            ...
            {plans.map((plan) => (
                <div 
                    ...
                    className={`relative p-10 lg:p-16 rounded-[64px] bg-white border border-stone-100 transition-all duration-500 h-full flex flex-col ${
                        plan.popular ? 'ring-4 ring-[#B55D3E]/10 shadow-3xl shadow-stone-200/50 scale-105 z-10' : 'hover:border-stone-200'
                    }`}
                >
                    {plan.popular && (
                        <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 px-8 py-3 bg-stone-900 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl shadow-2xl border border-white/10">
                            Institutional Choice
                        </div>
                    )}
                    
                    <div className="mb-12">
                        <div className="size-16 bg-stone-50 rounded-[20px] flex items-center justify-center mb-10 group-hover:bg-[#B55D3E] transition-colors">
                            <plan.icon className="size-8 text-[#B55D3E]" />
                        </div>
                        <h3 className="text-3xl font-black text-stone-900 mb-4 uppercase tracking-tighter">{plan.name}</h3>
                        <p className="text-stone-500 text-sm font-bold leading-relaxed italic">{plan.description}</p>
                    </div>

                    <div className="mb-12">
                        <div className="flex items-baseline gap-3">
                            <span className="text-7xl font-black text-stone-900 tracking-tighter">{plan.price}</span>
                            <div className="flex flex-col">
                                <span className="text-stone-400 font-black uppercase tracking-widest text-[10px]">EUR</span>
                                <span className="text-[#B55D3E] font-black uppercase tracking-widest text-[10px] tracking-tight">{plan.period}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6 mb-16 flex-grow">
                        {plan.features.map(feature => (
                            <div key={feature} className="flex gap-4 items-start">
                                ...
                                <span className="text-[11px] font-black text-stone-600 uppercase tracking-widest leading-relaxed">{feature}</span>
                            </div>
                        ))}
                    </div>
                    ...
                    <Link 
                        ...
                        className={`w-full py-6 rounded-[24px] font-black transition-all flex items-center justify-center gap-4 uppercase tracking-widest text-[10px] shadow-2xl ${
                            plan.popular 
                            ? 'bg-stone-900 text-white hover:bg-[#B55D3E] shadow-stone-900/20' 
        ...
        {/* Global Access Proof */}
        <div className="mt-32 p-12 md:p-24 bg-stone-950 rounded-[80px] text-white relative overflow-hidden border-b-8 border-[#B55D3E]">
            <div className="grid lg:grid-cols-2 gap-24 items-center relative z-10 text-left">
                <div className="space-y-10">
                    ...
                    <h2 className="text-4xl md:text-7xl font-black leading-[0.9] tracking-tight uppercase">High Velocity <br /><span className="text-[#B55D3E]">Global API.</span></h2>
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            ...
                            <div className="text-[10px] font-black text-[#B55D3E] uppercase tracking-widest">Managing Director, Mendes Global</div>
                        </div>
                    </div>
                </div>
                <div className="grid grid-cols-2 gap-8">
                    <div className="p-10 bg-white/5 rounded-[40px] border border-white/5 flex flex-col gap-6 hover:bg-white/10 transition-colors">
                        <Zap className="size-10 text-[#B55D3E]" />
