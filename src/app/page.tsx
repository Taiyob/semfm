          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
              <Link href="/calculator" className="btn-primary flex items-center gap-3">
                <Coins className="size-5" /> Found a property? Analyze returns
              </Link>
              <a href="#market-selection" className="btn-secondary flex items-center gap-3">
                Still searching? Find one <Home className="size-5 text-[#34495E]" />
              </a>
            </div>
          </motion.div>
        </div>
            ...
            <div className="grid gap-6">
              {[
                ...
                { 
                  ...
                  label: 'Initialize discovery protocol',
                ...
                { 
                  ...
                  label: 'Initialize intelligence protocol',
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                { 
                  step: '01', 
                  label: 'Discover', 
                  desc: 'Identify properties that match your investment strategy using smart filters and targeted search criteria.', 
                  icon: Search, 
                  color: 'bg-blue-50/50' 
                },
                { 
                  step: '02', 
                  label: 'Analyze', 
                  desc: 'Evaluate the returns of each property and determine whether it’s a strong fit for your investment goals.', 
                  icon: TrendingUp, 
                ...
                { 
                  step: '03', 
                  label: 'Execute', 
                  desc: 'Move forward with confidence — contact the agent and proceed with acquiring the property.', 
                  icon: Building2, 
                  color: 'bg-rose-50/50' 
                },
              ].map((item, i) => (
                <div 
                  key={item.step} 
                  className="bg-white p-10 rounded-[40px] border-2 border-transparent hover:border-[#D4A373]/30 shadow-xl shadow-stone-200/50 flex flex-col justify-between items-start relative group hover:-translate-y-2 transition-all duration-500 min-h-[240px]"
                >
                  <div className={cn("size-14 rounded-2xl flex items-center justify-center transition-all group-hover:bg-[#D4A373] group-hover:text-white", item.color)}>
                    <item.icon className="size-7 text-[#34495E] group-hover:text-white" />
                  </div>
                  <div className="relative z-10 mt-8">
                    <div className="text-2xl font-black text-[#2C3E50] group-hover:text-[#D4A373] transition-colors">{item.label}</div>
                    <p className="text-sm font-bold text-stone-400 leading-relaxed mt-4 group-hover:text-stone-500 transition-colors">
                      {item.desc}
                    </p>
                  </div>
                  <div className="text-6xl font-black text-[#2C3E50]/5 absolute bottom-6 right-8 group-hover:text-[#D4A373]/10 transition-colors pointer-events-none">
                    {item.step}
                  </div>
                  
                  {i < 2 && (
                    <div className="absolute -right-4 top-1/2 -translate-y-1/2 hidden md:block z-20 pointer-events-none opacity-20">
                      <ArrowRight className="size-6 text-[#34495E]" />
                    </div>
                  )}
                </div>
              ))}
          ...
          {filteredCountries.map((country, idx) => (
            ...
            >
              ...
              <div className="p-10 flex flex-col flex-grow space-y-8">
                <p className="text-stone-500 font-bold leading-relaxed">{country.description}</p>
                
                <div className="grid grid-cols-3 gap-3">
                  <div className="p-4 bg-stone-50 rounded-2xl flex flex-col items-center text-center">
                    <span className="text-[9px] font-black text-stone-400 uppercase tracking-widest mb-2">Appreciation</span>
                    <span className="text-lg font-black text-[#2C3E50]">{country.yield}</span>
                  </div>
                  <div className="p-4 bg-stone-50 rounded-2xl flex flex-col items-center text-center">
                    <span className="text-[9px] font-black text-stone-400 uppercase tracking-widest mb-2">Properties</span>
                    <span className="text-lg font-black text-[#2C3E50]">{country.availableProperties}</span>
                  </div>
                  <div className="p-4 bg-[#D4A373]/5 rounded-2xl flex flex-col items-center text-center border border-[#D4A373]/10">
                    <span className="text-[9px] font-black text-[#D4A373] uppercase tracking-widest mb-2">Gross Yield</span>
                ...
                {country.status === 'Active' ? (
                  <Link
                    ...
                    className="w-full mt-auto py-4 bg-slate-900 text-white font-bold rounded-2xl flex items-center justify-center gap-2 group/btn transition-colors hover:bg-[#34495E]"
                  >
